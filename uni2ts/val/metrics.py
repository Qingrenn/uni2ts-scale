...
def mean_weighted_seq_quantile_loss(data: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    for q in np.arange(0.1, 1, 0.1):
        stacked_quantile_losses.append(safe_div(quantile_loss(data, q), absolute_label(data)))
...
class ValMetric():
    ...
    def __call__(self, pred, target, observed_mask, prediction_mask, sample_id, variate_id) -> torch.Tensor:
        '''
...
class MeanWeightedSumQuantileLoss(ValMetric):    
    def __init__(self) -> None:
